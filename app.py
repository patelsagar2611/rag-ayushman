...
with st.sidebar:
    ...
    with st.popover("ℹ️ Demo version", use_container_width=True):
        st.markdown(
            ...
            "documents say.\n\n"
            "**The corpus is a snapshot taken in August 2026, and NHA has since "
            "published a newer edition of the empanelment guidelines that is not "
            "in it.** Empanelment answers are therefore drawn from superseded "
            "guidance, however correctly they cite it. Two older editions are in "
            "the corpus and they contradict each other on several rules — where "
            "that happens, open the cited pages rather than trusting the answer.\n\n"
...
def render_about():
    ...
    st.markdown(
        ...
        "say.\n\n"
        "**The corpus is a fixed snapshot from August 2026.** NHA has since "
        "published an empanelment edition newer than anything in it, so every "
        "empanelment answer here is drawn from superseded guidance — correctly "
        "cited, and out of date. Two older editions are in the corpus and they "
        "contradict each other on several rules; where that happens, open the "
        "cited pages rather than trusting the answer.\n\n"
        "A correct citation to an outdated rule is more convincing than a wrong "
        "answer and no easier to spot, which is why this is stated here rather "
